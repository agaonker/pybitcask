...
class Bitcask:
    ...
    def _build_index(self):
        ...
        for data_file in sorted(self.data_dir.glob("*.data")):
            with open(data_file, "rb") as f:
                while True:
                    # Read header
                    header = f.read(16)  # 4 bytes for key_size, 4 for value_size, 8 for timestamp
                    if not header:
                        break
                    ...
                    self.index[key] = (int(data_file.stem), value_size, value_pos, timestamp)
    ...
    def put(self, key: str, value: Any) -> None:
        ...
        with self._lock:
            ...
            # Update index
            self.index[key] = (self.active_file_id, len(value_bytes), 
                             self.active_file.tell() - len(value_bytes), timestamp)
    ...
    def delete(self, key: str) -> None:
        ...
        with self._lock:
            if key in self.index:
                ...
                tombstone = struct.pack(">IIQ", len(key.encode('utf-8')), 0, 
                                      int(time.time() * 1000))
